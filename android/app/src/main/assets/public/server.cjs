...
var sanitizeInput = (obj) => {
  if (typeof obj === "string") {
    if (obj.startsWith("data:") || obj.length > 1e4) {
      return obj;
    }
...
function sanitizeLaTeXInJSON(raw) {
  if (!raw) return raw;
  let inString = false;
  let isEscaped = false;
  let out = "";
...
${text}`.trim() },
  ...
  if (!isSpecialtyModel) {
    ...
    for (const m of modelsToTry) {
      ...
      const cooldownMs = Math.min(rateLimitedModelsCooldown[m] || 15e3, 3e4);
  ...
  for (const model of modelsToTry) {
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        const timeoutMs = params.timeoutMs && typeof params.timeoutMs === "number" ? params.timeoutMs : 3e4;
        let response;
        if (isAudioModel || isSpecialtyModel) {
          ...
          if (currentParams.config?.temperature !== void 0) generationConfig.temperature = currentParams.config.temperature;
          if (currentParams.config?.maxOutputTokens) generationConfig.maxOutputTokens = currentParams.config.maxOutputTokens;
          if (currentParams.config?.topP !== void 0) generationConfig.topP = currentParams.config.topP;
          if (currentParams.config?.topK !== void 0) generationConfig.topK = currentParams.config.topK;
        ...
        if (isRateLimitOrOverloaded) {
          ...
          if (isHardDailyQuota) {
            rateLimitedModelsCooldown[model] = 3e4;
    ...
    if (shouldStream) {
      ...
      for (const model of modelsToTry) {
        try {
          ...
          const timeoutPromise = new Promise(
            (_, reject) => setTimeout(() => reject(new Error(`Stream start timeout for model ${model}`)), 7e3)
          );
...
STUDENT PROFILE CONTEXT: ${profileContext}`;
    ...
    if (extractedText && extractedText.length > 8e4) {
      ...
      const introSlice = extractedText.slice(0, 3e4);
      const conclusionSlice = extractedText.slice(-2e4);
      const middleText = extractedText.slice(3e4, -2e4);
      let middleSamples = "";
      if (middleText.length > 4e4) {
        const chunkSize = 8e3;
        const totalChunks = 5;
...
async function performLiveWebSearch(query, searchKeywords = [], userCountry = "United States") {
  ...
  const queriesToSearch = Array.from(/* @__PURE__ */ new Set([primaryClean, ...searchKeywords])).filter((q) => q && q.length > 1).slice(0, 2);
...
- NEVER use generic textbook clich\xE9 questions (do NOT use Earth radius contraction by 1.5%, do NOT use Chlorine vs Fluorine electron gain enthalpy, do NOT use Krebs cycle NADH vs NADPH).
...
Return strictly a valid JSON object matching the requested schema with exactly ${requestedCount} items in the "questions" array.`;
      ...
      if (parsed && Array.isArray(parsed.questions) && parsed.questions.length > 0) {
        ...
        const seenInCurrentRun = /* @__PURE__ */ new Set();
        for (const q of parsed.questions) {
          if (q && q.question && typeof q.question === "string" && Array.isArray(q.options) && q.options.length === 4 && typeof q.correctIndex === "number" && q.correctIndex >= 0 && q.correctIndex < 4) {
