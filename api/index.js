...
async function robustFetchYoutubeTranscript(videoId) {
  ...
  try {
    const results2 = YoutubeTranscript.parseTranscriptXml(xmlText, selectedTrack.languageCode);
...
function getEducationalFallback(query, profileContext = "", studentNotes = "") {
  ...
  return {
    ...
    detailed_sources: canonicalLinks.map((uri) => ({
      title: uri.includes("wikipedia") ? "Wikipedia Academic Overview" : uri.includes("britannica") ? "Encyclopaedia Britannica" : uri.includes("khanacademy") ? "Khan Academy Learning Module" : uri.includes("sciencedirect") ? "ScienceDirect Research Papers" : uri.includes("scholar.google") ? "Google Scholar Articles" : uri.includes("ocw.mit.edu") ? "MIT OpenCourseWare" : uri.includes("nature.com") ? "Nature Scientific Journal" : "Verified Academic Source",
      uri
    }))
  };
}
...
Please perform a comprehensive live Google Search across 5 to 10+ authoritative websites, cross-verify the facts, and return the response strictly in JSON format according to our schema (NO markdown bolding '**').`;
    ...
    let extractedGroundingLinks = [];
    if (chunks && Array.isArray(chunks)) {
      extractedGroundingLinks = chunks.map((c) => ({
        title: c.web?.title || c.maps?.title || "Live Reference Source",
        uri: c.web?.uri || c.maps?.uri
      })).filter((item) => item.uri && typeof item.uri === "string" && item.uri.startsWith("http"));
    }
    ...
    const combinedUrls = [
      ...
      ...parsedResult.source_links.filter((u) => typeof u === "string" && u.startsWith("http"))
    ];
    const uniqueUrls = [];
