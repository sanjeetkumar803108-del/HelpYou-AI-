...
You are "Magic AI Tutor", an elite, highly intelligent, encouraging educational assistant, SAT/ACT Expert, and Master Educator.
...
3. RULE 3 (General Academic Explanations):
...
- MANDATORY ULTRA-CLEAN STEP-BY-STEP FORMATTING PROTOCOL (NO CLUSTERED TEXT):
  * LINE BREAK AFTER EVERY FULL STOP: Every single sentence, statement, or logical step must end with a period/full stop (.) and be followed by a double line break (\\n\\n). NEVER write dense, cluttered multi-sentence paragraphs.
...
3. RULE 3 (Humanities, Case Studies & Non-STEM Essays):
- Use this for general descriptive essays, humanities, case studies, historical events, or non-STEM conceptual questions (e.g., "French Revolution", "Who was George Washington?").
- Set "format_type" to "markdown".
- Output structured, rich text using standard markdown headings (###) and bullet points.
...
app.post("/api/chat", upload.single("image"), async (req, res) => {
  ...
  try {
    ...
    const academicTrack = (req.body.stream !== 'true' && req.body.stream !== true ? req.body.stream : null) || req.body.academicStream || req.body.academic_stream || req.body.track;
    const country = req.body.country || req.body.academic_country;
    const gradePedagogicalDirective = getGradePedagogicalDirective(gradeLevel, academicTrack, country);
...
[Explain mistakes and provide the perfect 10/10 mathematical solution tailored to their grade level]`;
    } else {
...
4. Use short paragraphs and emojis to make it visually friendly for beginners.`;
    ...
    const textPart = { text: `${gradeDirective}\n\n${promptText}` };
    ...
    if (action === 'flashcards-json') {
      let flashcards = safeParseJSON(outputText, 'array');
...
4. LATEX & CODE: If there are formulas, wrap in LaTeX ($...$). If coding/HTML tags, wrap in standard code formatting.
...
Generate exactly ${requestedCount} high-yield, comprehensive active recall revision flashcards covering the most critical concepts throughout the ENTIRE document from beginning to end tailored to Grade: ${gradeLevel || 'Standard'}.

CRITICAL ACTIVE RECALL RULES:
1. PUNCHY ACTIVE RECALL QUESTIONS: The 'question' must be direct, crisp, and test a single core mechanism, formula, definition, historical milestone, or concept from the document at their grade level.
2. STRICT 15 TO 25 WORDS ANSWER CONSTRAINT: Every 'answer' MUST be strictly concise, punchy, and between 15 to 25 words max. It must be an active recall mnemonic, definition, or key formula concept designed for rapid revision. NEVER output long multi-sentence paragraphs.
3. 100% COMPLETE THOUGHTS: The 15-25 word answer must be grammatically complete and self-contained (no trailing '...', no chopped clauses).
4. LATEX & CODE: If there are mathematical formulas, wrap in LaTeX ($...$). If coding/HTML tags, wrap in standard code formatting.
...
- DO NOT rewrite or fundamentally change the user's sentence structure, tone, vocabulary level, or core meaning. Keep it as close to the user's original words as possible, only correcting mistakes and very minor awkward phrasing.`;
      ...
      modeInstruction = `MODE: Academic Rewrite (Elevates vocabulary and flow appropriate to Grade: ${gradeLevel || 'Standard'})
- Elevate vocabulary, phrasing, and flow strictly matched to the student's grade level (${gradeLevel || 'Standard'}).
- For Middle School (Grades 6-8): elevate to crisp, clear 8th-grade academic English. NEVER introduce college-level jargon or dense polysyllabic phrasing.
- For High School / AP: elevate to AP/Board exam essay quality with formal academic transitions.
- For College: elevate to publication-grade scholarly discourse.
...
5. TONE & FORMAT:
   ...
   - Format cleanly in Markdown with bold headers and clear spacing.`;
    ...
    const userPrompt = followUpQuestion 
      ? `Original Question: ${questionText}\n${stimulus ? `Stimulus: ${stimulus}\n` : ''}${options && options.length > 0 ? `Options:\n${options.join('\n')}\n` : ''}\nStudent's Question to Tutor: "${followUpQuestion}"`
...
Your ONLY job is to generate a highly accurate, exam-level Multiple Choice Quiz for that topic tailored to the student's grade level (${gradeLevel || 'Standard'}).
...
2. FORMAT: Generate exactly ${requestedCount} questions. Each question must have exactly 4 options and a short explanation matching their grade level comprehension.
...
app.post("/api/generate-pdf-quiz", upload.single("pdf"), async (req, res) => {
  try {
    const { gradeLevel, count, stream, academic_stream, country, academic_country } = req.body;
    const academicTrack = stream || academic_stream;
    const countryContext = country || academic_country;
    const gradeDirective = getGradePedagogicalDirective(gradeLevel, academicTrack, countryContext);
    if (!req.file) {
...
[
  {
    ...
    if (extractedText && extractedText.trim().length >= 50) {
      ...
      response = await safeGenerateContent({
        ...
        model: "gemini-3.5-flash-lite",
        contents: [{
          parts: [{ text: `DOCUMENT CONTENT:\n${slicedText}\n\nGenerate the ${requestedCount}-question JSON quiz now based strictly on the content above for Grade: ${gradeLevel || 'Standard'}.` }]
        }],
        config: {
          systemInstruction: { parts: [{ text: systemInstruction }] },
          responseMimeType: "application/json"
      ...
      response = await safeGenerateContent({
        ...
        contents: [{ 
          parts: [
            pdfPart,
            { text: `Analyze the attached PDF document and generate the ${requestedCount}-question JSON quiz now based strictly on its content for Grade: ${gradeLevel || 'Standard'}.` }
          ] 
        }],
        config: {
          systemInstruction: { parts: [{ text: systemInstruction }] },
...
You are an expert exam creator and visual analyzer. Analyze the textbook page, question sheet, or study material in the provided image. Identify the key academic topics, concepts, or exercises shown on the page. Generate exactly ${requestedCount} multiple choice questions based strictly on the content of that textbook page tailored to Grade: ${gradeLevel || 'Standard'}.
    
CRITICAL RULES:
1. STRICT JSON OUTPUT: You must output ONLY a valid JSON array. Do not wrap it in markdown blockquotes like \`\`\`json. Absolutely ZERO conversational text before or after the JSON.
2. FORMAT: Generate exactly ${requestedCount} questions. Each question must have exactly 4 options (prefixed with A), B), C), D)) and a short explanation matching their grade level comprehension.
3. CORRECT ANSWER: The "correctAnswer" field MUST be a single string that EXACTLY matches one of the strings in the "options" array.
4. MULTIPLE EQUATIONS FORMATTING: If generating any math questions, options, or explanations that contain multiple equations (such as systems of linear equations), you must strictly separate the equations using a clear delimiter like the word 'and' or a newline character (\\n) so they do not blend together into a single string.
5. MATHEMATICAL & SCIENTIFIC NOTATION (LATEX):
   - Wrap ALL mathematical equations, expressions, variables, superscripts (exponents), and subscripts in standard single dollar signs ($...).
...
app.post("/api/fix-mistake", async (req, res) => {
  try {
    const { question, wrongInput, correctConcept, gradeLevel, stream, academic_stream, country, academic_country } = req.body;
    const academicTrack = stream || academic_stream;
    const countryContext = country || academic_country;
    const gradeDirective = getGradePedagogicalDirective(gradeLevel, academicTrack, countryContext);
...
Your job is to analyze a student's academic mistake and provide a structured 3-part conceptual breakdown tailored to Grade: ${gradeLevel || 'Standard'}.
...
app.post("/api/quiz-ai-help", async (req, res) => {
  try {
    const { question, options, correctAnswer, explanation, mode, gradeLevel, stream, academic_stream, country, academic_country } = req.body;
    if (!question) {
...
You are an elite SAT/AP/Olympiad Master Coach and Professor.
...
3. TONE & DEPTH:
   - Highly encouraging, intellectually rigorous, crystal-clear, and thorough matching their grade level. Never write lazy 1-sentence summaries.`;
...
app.post("/api/generate-practice", async (req, res) => {
  try {
    const { question, wrongInput, correctConcept, sourceFeature, gradeLevel, stream, academic_stream, country, academic_country } = req.body;
    const academicTrack = stream || academic_stream;
    const countryContext = country || academic_country;
    const gradeDirective = getGradePedagogicalDirective(gradeLevel, academicTrack, countryContext);
...
Your task is to generate exactly 3 multiple-choice practice questions that test the SAME core concept as the student's mistake, but with fresh numbers, contexts, or scenarios tailored to Grade: ${gradeLevel || 'Standard'}.
...
app.post("/api/generate-trivia", async (req, res) => {
  try {
    ...
    const studentGrade = gradeLevel || studyLevel || req.body.userGrade || "11th Grade";
    const studentStream = academicStream || req.body.stream || "STEM / Science & Engineering";
    ...
    // Determine 3-Card Subject & Trap Focus based on academic profile
    ...
    // Track-specific multi-set fallback database with zero-repetition guarantees
    const fallbackDatabase: Record<string, any[]> = {
      stem: [
        ...
        },
        {
          ...
          examTrapWarning: "Common mistake: Selecting u²/g by forgetting that speed at vertex is u cos θ, not initial speed u!"
        },
        {
          id: "stem_fb_5",
          subject: "Chemistry",
    ...
    // Helper to extract unseen fallback questions for user's stream
    const pickUnseenFallbackQuestions = (streamKey: string, count: number): any[] => {
      const primaryPool = fallbackDatabase[streamKey] || fallbackDatabase.stem;
      let unseen = primaryPool.filter(q => !isQuestionSeen(q.question));

      // If user has seen all in primary pool, combine with other pools
      if (unseen.length < count) {
        const otherPools = Object.entries(fallbackDatabase)
      ...
      // If still fewer than count (user has answered almost everything), shuffle and re-index
      const selected: any[] = [];
...
Return strictly a valid JSON object matching the requested schema with exactly ${requestedCount} items in the "questions" array.`;
      ...
      if (parsed && Array.isArray(parsed.questions) && parsed.questions.length > 0) {
        ...
        for (const q of parsed.questions) {
          if (
            ...
            // Strict non-repetition test: must NOT be in excludesSet and not duplicated in current batch
        ...
        // If we got enough valid unseen questions, we are done!
        if (validQuestions.length >= requestedCount) {
          ...
          // On last attempt, if we have some valid questions, supplement remaining with unseen fallback
    ...
    // Pick stream-specific, guaranteed non-repeating questions
    ...
    const normalizeStr = (s: string) => s ? s.toLowerCase().replace(/[^a-z0-9]/g, "") : "";
    const excludesSet = new Set((req.body.excludeQuestions || []).map((q: string) => normalizeStr(q)));
    ...
    // Grab all items from fallback database for this stream
    ...
    // If pool is small, supplement from other streams
    if (fallbackQuestions.length < requestedCount) {
      const others = Object.entries(fallbackDatabase)
        .filter(([k]) => k !== targetStreamKey)
        .flatMap(([, v]) => v)
        .filter(q => !isQuestionSeen(q.question));
