...
const summarizeStartIndex = lines.findIndex(l => l.includes('app.post("/api/summarize"'));
const ttsStartIndex = lines.findIndex(l => l.includes('app.post("/api/tts"'));

if (summarizeStartIndex !== -1 && ttsStartIndex !== -1) {
...
`;
  lines.splice(summarizeStartIndex, ttsStartIndex - summarizeStartIndex, newSummarize);
