...
export default function ContentGenerator({ onBack }: ContentGeneratorProps) {
  ...
  const handleGenerate = async () => {
    ...
    // Deduct 1 coin for AI Writing Helper
    if (!deductCoins(1, "AI Writing Helper")) {
      return;
    }
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      doc.text(`${selectedType} Study Guide - HelpYou AI`, margin, cursorY);
      cursorY += 10;
      
      doc.setFontSize(10);
      doc.setFont("helvetica", "italic");
      doc.text(`Topic: ${topic.substring(0, 70)}${topic.length > 70 ? '...' : ''}`, margin, cursorY);
      cursorY += 5;
      doc.text(`Tone: ${selectedTone} | Academic Format: ${selectedType === 'Essay' ? selectedFormat : 'Standard'}`, margin, cursorY);
      cursorY += 10;
      ...
      lines.forEach((line: string) => {
       ...
       await savePDFMobile(doc.output('blob'), filename);
     } catch (err) {
            ...
            <div className="flex items-center justify-between gap-3 mb-6 bg-white border border-zinc-200 rounded-2xl p-3 shadow-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={exporting}
                  className="flex items-center gap-1.5 px-3 py-2 bg-zinc-50 hover:bg-zinc-100 active:scale-95 border border-zinc-200 rounded-xl font-bold text-xs text-zinc-700 shadow-sm transition-all cursor-pointer select-none disabled:opacity-50"
                  title="Save as study PDF"
                >
                  {exporting ? (
                    <>
