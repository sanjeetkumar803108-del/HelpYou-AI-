...
export default function EssayGrader({ onBack }: { onBack: () => void }) {
  ...
  const [subject, setSubject] = useState('AP English Language');
  ...
  const handleCameraScan = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const response = await fetch((import.meta.env.VITE_API_BASE_URL || '') + '/api/scan-essay', {
        method: 'POST',
        body: formData,
      });
      ...
      if (data.text) {
        ...
        setErrorToast("No text could be extracted from the image. Please make sure the handwriting is clear.");
      }
    } catch (err: any) {
      console.error(err);
      setErrorToast("Failed to transcribe image. Please try typing or pasting your essay.");
    } finally {
      setScanning(false);
      // Reset input value
      e.target.value = '';
  ...
  const handleGrade = async () => {
    ...
    if (wordCount < 50) {
      setErrorToast("Please enter at least 50 words of your essay to get AP-level feedback.");
      return;
    }

    // Deduct 1 coin for AP Essay Grader
    if (!deductCoins(1, "AP Essay Grader")) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch((import.meta.env.VITE_API_BASE_URL || '') + '/api/grade-essay', {
        ...
        body: JSON.stringify({ text: essayText, subject, gradeLevel }),
      });
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-lg md:text-xl font-bold flex items-center tracking-tight line-clamp-1 text-zinc-900">
              ...
              <span>AP Essay Grader</span>
            </h2>
            <p className="text-[11px] text-zinc-500 font-medium line-clamp-1">AP-level grade & high-yield feedback</p>
          </div>
        </div>
      ...
      {showHistory ? (
        <div className="max-w-md mx-auto space-y-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs bg-zinc-100 text-zinc-600 font-bold px-2 py-0.5 rounded-full">{historyItems.length} graded</span>
          </div>
        ...
        <div className="flex-1 flex flex-col">
          ...
          <div className="mb-4 relative">
            <label className="block text-xs font-black text-zinc-400 uppercase tracking-widest mb-2">Select AP Subject</label>
            <div className="relative">
              <button
                type="button"
                onClick={() => !loading && !scanning && setIsDropdownOpen(!isDropdownOpen)}
              ...
              >
                <span>{subject}</span>
                <svg className={`fill-current h-4 w-4 text-zinc-500 transition-transform duration-200 ${isDropdownOpen ? 'rotate-180' : ''}`} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
          ...
          <div className="mb-6">
            ...
            <button
              type="button"
              onClick={() => document.getElementById('essay-camera-input')?.click()}
              disabled={scanning || loading}
              className="w-full flex items-center justify-center gap-2.5 px-5 py-3.5 rounded-2xl bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-100/50 font-extrabold text-sm transition-all active:scale-[0.99] disabled:opacity-50"
            >
              {scanning ? (
                <>
                  <Loader2 className="w-4 h-4 animate-spin text-indigo-600" />
                  <span>Transcribing handwritten essay...</span>
                </>
              ) : (
                <>
                  <Camera className="w-4 h-4" />
                  <span>📷 Scan Handwritten Essay</span>
                </>
              )}
            </button>
          </div>
