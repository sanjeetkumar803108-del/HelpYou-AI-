...
import { deductCoins, isProUser } from '../utils/coins';
...
const CURRICULUM_DATA = [
  { 
    id: "sat_math", 
    title: "SAT Math", 
    subtitle: "Advanced Algebra & Geometry", 
  ...
  { 
    ...
    subtitle: "Evidence-Based Reading", 
  ...
  { 
    id: "ap_ushistory", 
    title: "AP US History", 
    subtitle: "American Revolution to Cold War", 
    themeColor: "bg-red-50/70 text-red-600 border-red-100", 
    topics: ["Colonial America", "The American Revolution", "Civil War", "Cold War Era"] 
  }
];
...
export default function QuizGenerator({ onBack }: { onBack: () => void }) {
  ...
  const fetchHistory = async () => {
    if (!auth.currentUser) {
      ...
      try {
        ...
        const parsed = localItems.map((item: any) => ({
          ...item,
    ...
    try {
      ...
      const combined = [...localItems, ...items].sort((a, b) => a.createdAt.getTime() - b.createdAt.getTime());
  ...
  const handlePDFUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Deduct 2 coins for AI Quizzes
    if (!deductCoins(2, "AI Quizzes")) {
    ...
    // Set a friendly clean name as the topic
  ...
  const handleGenerate = async (selectedTopic: string) => {
    ...
    try {
      ...
      const response = await fetch((import.meta.env.VITE_API_BASE_URL || '') + '/api/generate-quiz', {
        ...
        body: JSON.stringify({ topic: activeTopic, gradeLevel }),
      });
    ...
    if (currentIndex + 1 < quiz.length) {
      ...
      if (auth.currentUser) {
        try {
          ...
          console.error("Auto-saving quiz result failed:", e);
        }
      } else {
        try {
          ...
          console.error("Saving local quiz history failed:", e);
        }
      }
  ...
  return (
    <div className="h-full flex flex-col relative text-zinc-900 bg-[#FAF9F6] overflow-hidden">
      {(loading || pdfProcessing || photoProcessing) && quiz.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center py-12 px-6 text-center max-w-md mx-auto z-40">
          <div className="relative mb-8">
            <div className="absolute inset-0 bg-indigo-500/10 rounded-full blur-xl animate-pulse" />
            <div className="relative w-20 h-20 bg-gradient-to-tr from-indigo-500 to-blue-600 rounded-3xl flex items-center justify-center shadow-xl shadow-indigo-500/20">
              <Sparkles className="w-10 h-10 text-white animate-pulse" />
            </div>
          </div>

          <h3 className="text-2xl font-black text-zinc-900 tracking-tight mb-2">
            {pdfProcessing ? "Analyzing Document..." : photoProcessing ? "Reading Photo..." : "Generating Quiz..."}
          </h3>
          <p className="text-indigo-600 font-bold text-sm tracking-wide uppercase mb-8 min-h-[20px]">
            {quizSteps[loadingStep]}
          </p>
          ...
          <div className="w-full space-y-4">
             ...
             <div className="flex items-center gap-4 bg-white/30 p-4 rounded-2xl border border-zinc-100 animate-pulse scale-95 opacity-60">
                ...
                <div className="h-4 bg-zinc-50 rounded-full w-[80%]" />
             </div>
          </div>
        </div>
      )}
            ...
            {activeTab === 'quizzes' && (
              <>
                ...
                <div className="mb-6">
                      ...
                      <button
                        ...
                        className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all shrink-0 ${
                          isListening
                            ...
                            : 'bg-zinc-100 border-zinc-200 text-zinc-600 hover:bg-zinc-200 hover:text-zinc-800 shadow-sm'
                        }`}
                  ...
                  {isListening && (
                    <div className="flex items-center gap-2 mt-2.5 px-1.5 text-rose-500 font-bold text-xs animate-pulse">
                      ...
                      <span>Listening... Dictate your topic (e.g., "AP Biology Cellular Energetics")</span>
                    </div>
                  )}
                </div>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-bold text-base shadow-sm ${subject.themeColor}`}>
                                {subject.title === 'SAT Math' ? '📐' : 
                                 subject.title === 'SAT Reading & Writing' ? '📖' : 
                                 subject.title === 'AP Biology' ? '🧬' : '🇺🇸'}
                              </div>
                              <div>
                                <h4 className="font-extrabold text-sm text-zinc-800 leading-snug">
                                  {subject.title}
                ...
                {!deepFocus && (
                <div className="bg-indigo-50 border border-indigo-100 p-5 rounded-3xl flex items-start gap-4 shadow-sm">
                  ...
                  <div>
                    <h4 className="font-extrabold text-sm text-indigo-900">Earn Study Coins!</h4>
                    <p className="text-xs text-indigo-700/80 mt-1 leading-relaxed font-bold">
                      Complete any exam-level quiz to earn coins and level up your status. Let\'s test your high school curriculum skills!
                    </p>
                  </div>
                </div>
                )}
              ...
              <div className="flex-1 flex flex-col space-y-6">
                  ...
                  <div className="bg-white border border-zinc-200/80 rounded-3xl p-4 shadow-sm flex flex-col items-center text-center justify-between">
                    ...
                    <div>
                      ...
                      <h5 className="text-xl font-black text-zinc-800 mt-0.5">
                        {history.length === 0 ? DEMO_HISTORY.length : history.length}
                      </h5>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-zinc-200/80 rounded-3xl p-4 shadow-sm flex flex-col items-center text-center justify-between">
                    ...
                    <div>
                      ...
                      <h5 className="text-xl font-black text-emerald-600 mt-0.5">
                        {(() => {
                          const list = history.length === 0 ? DEMO_HISTORY : history;
                ...
                <div className="bg-white border border-zinc-200/80 rounded-[2rem] p-5 shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-[9px] font-black uppercase bg-zinc-100 text-zinc-500 px-2 py-0.5 rounded-md">
                      {history.length === 0 ? 'Diagnostic Data' : 'Live Results'}
                    </span>
                  </div>
                  ...
                  <div className="w-full h-64 md:h-72">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart
                        data={(history.length === 0 ? DEMO_HISTORY : history).map((item, index) => {
                ...
                <div className="bg-white border border-zinc-200/80 rounded-[2rem] p-5 shadow-sm">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {(history.length === 0 ? DEMO_HISTORY : history).slice().reverse().map((item) => {
                      const list = history.length === 0 ? DEMO_HISTORY : history;
                      const activeQuiz = selectedHistoryItem || (list.length > 0 ? list[list.length - 1] : null);
                ...
                <div className="w-full aspect-square max-w-[320px] rounded-3xl overflow-hidden border border-zinc-800 bg-zinc-900 shadow-inner flex items-center justify-center relative group mb-6">
                  {shareImageUrl ? (
                    ...
                    <div className="flex flex-col items-center justify-center text-zinc-500 p-4">
                      ...
                      <p className="text-xs font-semibold">Generating high-definition card...</p>
                    </div>
                  )}
                </div>
