...
import { ArrowLeft, Loader2, Save, FileText, Upload, History, Trash2, Calendar, Share2, Download, Flag } from 'lucide-react';
...
export default function Summariser({ onBack }: SummariserProps) {
    ...
    if (loading && !result) {
      ...
      interval = setInterval(() => {
        setLoadingProgress((prev) => {
          ...
          const stepIndex = Math.min(Math.floor((nextVal / 96) * (summarisingSteps.length - 1)), summarisingSteps.length - 1);
  ...
  const handleSummarise = async () => {
    ...
    try {
      ...
      setResult(data.text);
      const topicSnippet = inputText.trim().replace(/\s+/g, ' ').substring(0, 30);
      const docTitle = topicSnippet ? `Summary: ${topicSnippet}` : `Text Summary (${format === 'tldr' ? 'TL;DR' : format === 'eli5' ? 'ELI5' : 'Bullets'})`;
  ...
  const handleShareText = async () => {
    ...
    triggerVibration(10);
    const title = currentActiveTitle || 'Summary';
    await shareTextMobile(`Summary: ${title}`, result);
  };
  ...
  const processFile = async (file: File) => {
    ...
    if (profile.profileContext) formData.append('profileContext', profile.profileContext);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errMsg = errText.substring(0, 150) || errMsg;
        }
        throw new Error(errMsg);
      }
      ...
      if (!data.text) {
        throw new Error("No summary generated from this document. Please check the file content.");
      }
            ...
            <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-md">
              ...
              <div className="relative mb-3">
                <textarea
                  ...
                  placeholder="Paste notes, article text, or type any topic (e.g. Ohm's Law, Cell Division)..."
              ...
              <div className="flex flex-col gap-2 mb-6">
                <button
                  ...
                  className="w-full py-2.5 px-4 rounded-xl border border-zinc-200 bg-zinc-50 hover:bg-zinc-100 active:scale-[0.99] text-zinc-700 hover:text-zinc-900 text-xs font-bold flex items-center justify-center gap-1.5 transition-all cursor-pointer"
                >
          ...
          >
            ...
            <div className="space-y-3">
              <button
                ...
                className="w-full py-4 rounded-xl font-bold text-base shadow-lg active:scale-[0.98] transition-all flex items-center justify-center gap-2 border border-emerald-500/20 text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 shadow-emerald-500/10 cursor-pointer"
              >
                <Share2 className="w-5 h-5 text-white" />
                <span>Share PDF Summary</span>
              </button>
